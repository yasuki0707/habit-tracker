...
export const retrieveNotionDatabaseId = async () => {
  ...
  if (!data) {
    console.error(`Document ${NOTION_DOC_ID} is not included in Firestore:${COLLECTION_PATH}`);
    return null;
  }
  const databaseId = data['databaseId'];
  if (!databaseId) {
    console.error(`Field 'databaseId' is not included in Firestore:${COLLECTION_PATH}/${NOTION_DOC_ID}`);
    return null;
  }
...
export const storeDatabaseId = async (newDatabaseId: string) => {
  await firestore().collection(`${COLLECTION_PATH}`).doc(NOTION_DOC_ID).set({ databaseId: newDatabaseId });
};
