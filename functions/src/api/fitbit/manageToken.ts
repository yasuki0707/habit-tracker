...
export const retrieveRefreshToken = async () => {
	...
	if (!data) {
		console.error(
			`Document '${FITBIT_DOC_ID}' is not included in Firestore: '${COLLECTION_PATH}'`,
		);
		return null;
	}
	...
	if (!refreshToken) {
		console.error(
			`Field 'refreshToken' is not included in Firestore: '${COLLECTION_PATH}/${FITBIT_DOC_ID}'`,
		);
		return null;
	}
...
export const issueTokenPair = async (refreshToken: string) => {
	...
	if ('success' in data && !data.success) {
		console.error(
			`Failed to fetch refresh token from firestore: ${data.errors[0].message}`,
		);
		return null;
	}
