...
export const fetchData = async (dateForFitbit: string, accessToken: string) => {
  ...
  const totalDistance = activity.summary.distances.find((d: any) => d.activity === 'total').distance;
  const running = {
    steps: activity.activities.find((act: any) => act.name === 'Run')?.steps || 0,
    durationInMin: activity.activities.find((act: any) => act.name === 'Run')?.duration / 1000 / 60 || 0,
  };
